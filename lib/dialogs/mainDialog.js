...
class MainDialog extends botbuilder_dialogs_1.ComponentDialog {
    ...
    run(context, accessor) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (text) {
                case 'BOOK_ROOMS':
                    yield this.startBooking(context);
                    break;
                case 'GET_STARTED':
                    yield this.mainMenu(context);
                case 'HOTEL_INFO':
                    yield this.hotelInfo(context);
                    break;
                case 'CALL_HOTEL':
                    yield this.callHotel(context);
                    break;
                default:
                    break;
            }
            if (text && text.startsWith('SHOW_FACILITY')) {
                yield this.detailFacility(context);
            }
        });
    }
